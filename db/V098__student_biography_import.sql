-- ═══════════════════════════════════════════════════════════════════════════
-- V098 — migrate full student biography from the old portal
--
--   The old portal exports one wide row per student: the core record (matric,
--   name, programme, level), the contact (phone, email, address), and the rich
--   biography (origin, guardian, sponsor, next-of-kin, extracurricular). This
--   loads all of it in one pass, keyed on the matriculation number.
--
--   Three things it does that the plain student importer (people.import_students)
--   does not:
--     1. accepts an old-portal matric AS-IS. The University's own numbers are
--        MOAUM/DEPT/YY/NNNN; a migrated student keeps the number the previous
--        institution issued (e.g. BSU/BM/RAD/21/2204), so the matric CHECK is
--        widened to admit a legacy shape as well. New matriculation still mints
--        only MOAUM numbers — that logic is untouched.
--     2. writes the rich biography into people.biodata (the key/value store) and
--        the contact into people.student_contact.
--     3. provisions a matric sign-in account (iam.student_account) with an
--        unknown random password and must_change — the student activates it
--        through the ordinary reset, delivered to the email/phone imported here.
--        No password is ever taken from the file.
--
--   Idempotent: re-uploading the same export updates rather than duplicates, and
--   a blank cell never wipes a value that is already on the record.
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1 · admit a legacy matriculation number alongside the University's own ──
...
ALTER TABLE people.student ADD CONSTRAINT ck_student_matric_shape CHECK (
    matric_no IS NULL
    OR matric_no ~ '^MOAUM/[A-Z]{2,4}/[0-9]{2}/[0-9]{4}$'          -- the University's own
    OR matric_no ~ '^[A-Z]{2,6}(/[A-Z0-9]{2,6}){1,3}/[0-9]{2,7}$'  -- a legacy old-portal number, kept as issued
);

-- ── 2 · the biography fields the old portal carries that the dictionary lacked ──
INSERT INTO ref.biodata_field (field, section, label, tier, hint, wide, ord) VALUES
    ('sponsor_name',    'origin',   'Sponsor — name',    'open', 'The person or body responsible for the fees, where not a parent or guardian', false, 39),
    ('sponsor_address', 'origin',   'Sponsor — address', 'open', NULL, true, 40),
    ('extracurricular', 'personal', 'Extracurricular / hobbies', 'open', 'Sport, clubs and interests declared on the record', true, 18),
    ('legacy_appno',    'personal', 'Legacy application number',  'open', 'The application number the previous portal issued; kept for provenance', false, 9)
ON CONFLICT (field) DO NOTHING;

-- ── 3 · the importer ────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION people.import_biography(p_rows jsonb)
RETURNS TABLE (rows int, created int, updated int, no_programme int, bad_number int, contacts int, biography int, accounts int)
LANGUAGE plpgsql AS $$
DECLARE
    r jsonb;
    v_actor uuid := nullif(current_setting('moaum.actor_id', true), '')::uuid;
    v_matric text; v_prog text; v_prog_code text; v_sex text; v_dob date;
    ...
    ncontact int := 0; nbio int := 0; nacct int := 0;
BEGIN
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        -- the matric must be the University's own shape or an accepted legacy shape
        IF v_matric !~ '^MOAUM/[A-Z]{2,4}/[0-9]{2}/[0-9]{4}$'
           AND v_matric !~ '^[A-Z]{2,6}(/[A-Z0-9]{2,6}){1,3}/[0-9]{2,7}$' THEN
            nbn := nbn + 1; CONTINUE;
        END IF;

        -- the programme resolves by code first, then by name (dept and faculty follow from it)
        ...
        -- the name: surname + other names, or a single "SURNAME, Others" / "Others SURNAME" split
        ...
        IF v_es IS NULL OR v_es !~ '^[0-9]{4}/[0-9]{4}$' THEN
            -- the University's own number carries the entry year in its third segment; a legacy number does not
        ...
        VALUES (gen_random_uuid(), v_matric, v_surname, v_others, v_sex, v_dob, v_state, v_prog_code, v_mode,
        ...
        -- ── the contact (people.student_contact): a Nigerian number 0XXXXXXXXXX, a valid email, an address ──
        ...
        -- ── the rich biography (people.biodata): one row per field that is present ──
        FOR bf, bv IN
        ...
        -- ── the sign-in account: provisioned once, with an unknown password to be reset by the student ──
    ...
    RETURN QUERY SELECT n, nc, nu, nnp, nbn, ncontact, nbio, nacct;
END $$;

COMMENT ON FUNCTION people.import_biography(jsonb) IS
  'Loads a wide old-portal biography export in one pass: student core + contact + biodata + a matric sign-in '
  'account. Keyed on matric_no; idempotent; a blank cell never wipes a value already on the record; no password '
  'is taken from the file. Returns the counts row.';
