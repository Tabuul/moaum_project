-- ═══════════════════════════════════════════════════════════════════════════
-- V134 — platform.remove_demo_data: include matriculated demo-candidate students
--
--   V133 cleared the FK children, so the purge reached admissions.candidate and
--   hit student_candidate_id_fkey: a people.student row referenced a demo
--   candidate. The demo student inserts leave candidate_id NULL and the demo
--   admission cohort only runs to ACCEPTED, but walking that cohort through
--   matriculation IN THE PORTAL creates a real student row carrying
--   candidate_id -> a demo candidate. Its surname is 'DEMO' (carried from the
--   candidate) but its matric is a normal matriculation number, so the narrow
--   _ds pattern (…/990[1-6]) missed it, and it blocked the candidate delete.
--
--   candidate_id IN (the demo candidates) is an unambiguous demo marker (their
--   jamb_reg_no is 202699…D_, a range real candidates never fall in), so _ds is
--   widened to include those students. They are then purged with all their
--   children by the same statements, and the candidate delete is unblocked.
--   Everything else is identical to V133. V133 is applied, so this is a new file.
-- ═══════════════════════════════════════════════════════════════════════════
    ...
    CREATE TEMP TABLE _ds ON COMMIT DROP AS
        SELECT id FROM people.student
         WHERE (surname = 'DEMO' AND matric_no ~ '^MOAUM/[A-Z]{2,6}/[0-9]{2}/990[1-6]$')
            -- a student matriculated (in the portal) from a demo admission candidate:
            -- the candidate's jamb_reg_no is 202699…D_, a demo-only range
            OR candidate_id IN (SELECT id FROM admissions.candidate WHERE jamb_reg_no LIKE '202699%D_');
