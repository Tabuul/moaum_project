-- ═══════════════════════════════════════════════════════════════════════════
-- V177 — revert the migration hash to cost 12 (a check constraint requires it)
--
--   V176 lowered the initial-password hash to bcrypt cost 10 to speed the import,
--   but every account row is guarded by
--       ck_applicant_hash CHECK (password_hash LIKE '$2%$12$%')
--   which mandates cost 12 across all credentials (staff, applicants, students).
--   A cost-10 hash ('$2a$10$…') fails that constraint, so every matching row
--   errored. Cost 12 is the University's credential policy; it is not lowered.
--   This restores gen_salt('bf', 12); import speed comes from parallel uploads
--   and splitting the file, not from a weaker hash. Everything else in
--   import_applicant is identical to V174/V176.
-- ═══════════════════════════════════════════════════════════════════════════
