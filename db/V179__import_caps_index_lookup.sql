-- ═══════════════════════════════════════════════════════════════════════════
-- V179 — the import's CAPS lookup uses the index (this is why it was slow)
--
--   import_applicant matched the CAPS row with
--       WHERE r.session = p_session AND upper(btrim(r.jamb_reg_no)) = key
--   Wrapping the column in upper(btrim(...)) means the (session, jamb_reg_no)
--   index cannot be used, so EACH imported row sequentially scanned all ~229,000
--   CAPS rows — 13,000 rows x 229,000 = billions of comparisons, hence hours with
--   no visible progress. caps_row already has a STORED generated column
--   jamb_key = upper(btrim(jamb_reg_no)) with a unique index (session, jamb_key).
--   Matching r.jamb_key = key (the file's key is already upper(btrim(...))) is a
--   single index lookup per row, so the import runs at insert speed. Only the
--   lookup predicate changes; everything else is identical to V178.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- index lookup on the generated jamb_key (session, jamb_key) — not a scan of jamb_reg_no
