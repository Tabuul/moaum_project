-- ═══════════════════════════════════════════════════════════════════════════
-- V117 — store the old-portal school id, and derive the curriculum from it
--
--   The old portal groups students by a school id: S001 and S003 are the
--   undergraduate schools (CCMAS from 2023/2024, BMAS before), S002 is the
--   postgraduate school (BMAS). mode_entry does not distinguish postgraduates,
--   so the school id is the signal. We store it on the student and derive the
--   curriculum from it: postgraduate (S002) → BMAS; otherwise by entry session.
--   The biography and students imports now read a "School Id" column.
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

ALTER TABLE people.student ADD COLUMN IF NOT EXISTS school_id text;

-- postgraduate (S002) sits under BMAS; undergraduate by entry session (CCMAS from 2023/2024)
CREATE OR REPLACE FUNCTION people.curriculum_for(p_session text, p_school_id text)
...
-- fill the curriculum on insert, and recompute it when the entry session or school id changes
CREATE OR REPLACE FUNCTION people.fill_curriculum() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF TG_OP = 'INSERT'
       OR NEW.curriculum_version IS NULL
       OR NEW.entry_session IS DISTINCT FROM OLD.entry_session
       OR NEW.school_id IS DISTINCT FROM OLD.school_id THEN
        NEW.curriculum_version := people.curriculum_for(NEW.entry_session, NEW.school_id);
    END IF;
    RETURN NEW;
END $$;
...
DECLARE
    ...
    v_state text; v_phone text; v_email text; v_addr text; v_school text; v_student uuid; v_exists boolean;
...
BEGIN
    ...
    IF p_rows IS NULL OR jsonb_typeof(p_rows) <> 'array' OR jsonb_array_length(p_rows) = 0 THEN
        RAISE EXCEPTION 'the file is rows: matriculation number, name, programme, level, and the biography' USING ERRCODE = '23514';
    END IF;
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        BEGIN
            ...
            v_mode := CASE v_mode WHEN 'DE' THEN 'DIRECT_ENTRY' WHEN 'DIRECT ENTRY' THEN 'DIRECT_ENTRY' WHEN 'PG' THEN 'POSTGRADUATE' ELSE v_mode END;
...
-- ── the core students import also reads and stores the school id ──
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        BEGIN
            ...
            INSERT INTO people.student (id, matric_no, surname, other_names, sex, date_of_birth, state_of_origin, programme_code, entry_mode,
                                        entry_session, entry_level, current_level, status, matriculated_at, school_id)
