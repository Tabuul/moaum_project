-- ═══════════════════════════════════════════════════════════════════════════
-- V221 — accept the postgraduate matriculation-number format on import
--
--   A postgraduate matriculation number is longer than the undergraduate one —
--   e.g. MOAU/AM/BSM/PGDM/MSC/25/0021, MOAU/SS/PSY/PGD/24/0002, BSU/ED/EDF/
--   PGDE/23/0472 — with the faculty, department and programme spelled out. The
--   old check (V203) allowed at most four segments, so it rejected these as a
--   "bad matric format". This relaxes the check to any institutional number
--   (a two-to-six letter prefix such as MOAU, MOAUM or BSU, then two to eight
--   slash-separated segments), and derives the entry session from the YY that
--   precedes the serial at the end, wherever it falls. Everything else is V203.
-- ═══════════════════════════════════════════════════════════════════════════
...
DECLARE
    ...
    v_es text; v_yy text; v_el int; v_cl int; v_surname text; v_others text; v_school text;
...
BEGIN
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        -- any institutional matriculation number: a 2–6 letter prefix (MOAU / MOAUM / BSU …) then 2–8 segments
        IF v_matric !~ '^[A-Z]{2,6}(/[A-Z0-9]{1,12}){2,8}$' THEN
            nbn := nbn + 1; CONTINUE;
        END IF;
        ...
        BEGIN
            ...
            -- entry session from the file, else from the YY before the serial at the end of the matric
            v_es := nullif(btrim(coalesce(r->>'entrySession', r->>'entry_session', r->>'yoe', '')), '');
            IF v_es IS NULL OR v_es !~ '^[0-9]{4}/[0-9]{4}$' THEN
                v_yy := substring(v_matric from '/([0-9]{2})/[0-9]{2,7}$');
                IF v_yy IS NOT NULL THEN
                    v_es := '20' || v_yy || '/20' || lpad((v_yy::int + 1)::text, 2, '0');
                ELSE
                    v_es := '2024/2025';
                END IF;
            END IF;
