-- ═══════════════════════════════════════════════════════════════════════════
-- V111 — the biography import survives one bad row, and admits more legacy shapes
--
--   A real 45,000-row old-portal export carries stragglers: a matriculation
--   number with an extra segment (the mode, e.g. BSU/SC/CMP/UT/23/71773), a
--   value that trips a constraint. V098 skipped a bad matric or an unknown
--   programme and counted it, but any OTHER error on a row aborted the whole
--   batch. This wraps each row in a savepoint so a single row is skipped and
--   counted — with the first error kept, so the officer sees why — and the
--   rest of the batch still loads. The legacy matric shape is also widened to
--   admit a fourth segment, which the mode-carrying numbers use.
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1 · admit the mode-carrying legacy matric (a fourth segment) ──
...
-- ── 2 · the resilient importer (adds skipped + first_error to the count) ──
DROP FUNCTION IF EXISTS people.import_biography(jsonb);
...
BEGIN
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        -- everything that touches the database is one savepoint: a row that trips a
        -- constraint is rolled back on its own, counted, and the batch goes on
        ...
        BEGIN
            ...
            -- counted only once the whole row has landed
...
COMMENT ON FUNCTION people.import_biography(jsonb) IS
  'Loads a wide old-portal biography export in one pass, each row in its own savepoint so a single '
  'bad row is skipped and counted (skipped, with first_error) instead of aborting the batch; core + '
  'contact + biodata + a matric sign-in account, keyed on the matriculation number.';
