-- ═══════════════════════════════════════════════════════════════════════════
-- V121 — legacy results: any bad row is skipped, the batch never rolls back
--
--   V093 already skips an out-of-range mark, an unmatched student and an
--   unknown course. But any OTHER error in a row — a duplicate the ledger has
--   not anticipated, a constraint, a bad number — still aborted the whole
--   batch, so a 228k-row upload could refuse at row 51,749 and lose the batch.
--
--   This wraps each row in its own savepoint: on any unexpected error the row
--   is set aside (counted under "skipped", with the first message kept), and
--   the rest of the batch loads. Re-uploading is safe — rows already in are
--   left as they are. Two columns are added to the return (skipped,
--   first_error); the caller reads the row as a map, so nothing downstream
--   breaks.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        BEGIN   -- one savepoint per row: any unexpected error sets the row aside, the batch survives
            ...
            -- skip and report an out-of-range mark rather than letting the constraint abort the row
