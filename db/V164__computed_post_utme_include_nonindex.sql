-- ===========================================================================
-- V164 - computed Post-UTME shows every non-index (and Direct Entry) applicant
--
--   A non-sitter is a Direct Entry entrant OR anyone in a programme not screened
--   by examination — they never sit the Post-UTME. The previous condition also
--   required screening_score IS NULL, so a non-index applicant who had a stray
--   Post-UTME score uploaded (from when the programme was wrongly treated as
--   exam-screened) was hidden. Base it on the programme/entry mode instead: a
--   non-index programme or a Direct Entry candidate is a non-sitter regardless of
--   any stray score. Exam-screened (index) programmes' UTME candidates are still
--   excluded — their scores are uploaded, not computed.
-- ===========================================================================
...
LANGUAGE sql STABLE AS $$
    ...
    SELECT jamb_reg_no, cand_name, prog_name, mode, ol_total, ceiling,
           ...
           CASE WHEN utme_agg IS NOT NULL AND ol_total > 0 THEN 'O''Level + UTME'
                ...
                ELSE 'none' END
      FROM scored
     WHERE NOT by_exam OR mode = 'DIRECT_ENTRY'        -- non-index programme, or Direct Entry (never sits)
