-- ═══════════════════════════════════════════════════════════════════════════
-- V132 — fix platform.remove_demo_data: hostel.allocation has no student_id
--
--   V130 fixed the paydirect line, but the next landmine was hostel.allocation,
--   which has no student_id column — a bed is tied to a student through
--   application_id → hostel.application.student_id. Two statements filtered it
--   by student_id (its own DELETE, and the maintenance_request sub-select),
--   both raising the same "column \"student_id\" does not exist" the first time
--   the purge is called. CI never catches this: db/demo.sh seeds the demo data
--   but never CALLS the purge, so the function body is only validated at run
--   time on the live database — statement by statement, failing at the first
--   bad column, which is why fixing one line only revealed the next.
--
--   This audits every student_id filter in the function; hostel.allocation was
--   the last one left. It is now matched through hostel.application (still
--   present at that point — the application rows are deleted later in the same
--   transaction). Everything else is identical to V130. V130 is applied, so the
--   correction goes in this new file, not by editing V130.
-- ═══════════════════════════════════════════════════════════════════════════
