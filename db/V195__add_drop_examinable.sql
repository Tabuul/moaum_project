-- ═══════════════════════════════════════════════════════════════════════════
-- V195 — a course added during add/drop is examinable at once
--
--   V139's student_add kept an added course as REGISTERED "a live entry the
--   register counts". But the score sheet's roll, the lecturer's score-entry
--   roster, the candidate counts and the advance-completeness gate all count
--   only APPROVED entries — so an added course showed on the exam docket and
--   card (which count REGISTERED + APPROVED) yet had no row for the lecturer to
--   score. The student sat a paper that could not be recorded.
--
--   Add/drop is student self-service within the window, already gated on the
--   offer, units, clearance and the level maximum; there is no further approval
--   step. So when the registration is already APPROVED, the added entry becomes
--   APPROVED too — examinable and counted everywhere. On a still-SUBMITTED
--   registration it stays REGISTERED, and the Head of Department's approval
--   flips it to APPROVED with the rest of the batch, exactly as before.
-- ═══════════════════════════════════════════════════════════════════════════
...
RETURNS int
LANGUAGE plpgsql AS $$
DECLARE r registration.course_registration; m record; lim policy.level_limit; v_units int; v_type text; v_status text;
BEGIN
...
COMMENT ON FUNCTION registration.student_add(uuid, text, int, uuid) IS
  'Add a course to a submitted/approved registration during the add/drop window (units, clearance and level max '
  'enforced; a dropped course is reinstated). On an approved registration the added course is APPROVED at once so '
  'it is examinable and counted; on a submitted one it stays REGISTERED until the HOD approves the batch.';
