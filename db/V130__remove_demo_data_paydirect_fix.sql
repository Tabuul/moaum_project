-- ═══════════════════════════════════════════════════════════════════════════
-- V130 — fix platform.remove_demo_data: paydirect_collection has no student_id
--
--   V129 purged the demo operational data, but one line read
--       DELETE FROM finance.paydirect_collection WHERE student_id IN (…)
--   and finance.paydirect_collection has no student_id column — a settlement
--   row is tied to a student only through the PRN / reference the portal
--   generated. A plpgsql body is validated on first call, so V129 applied
--   cleanly and the error only surfaced the first time the purge was run
--   (column "student_id" does not exist). V129 is already applied, so the
--   correction goes in this new file, not by editing V129.
--
--   Only that one statement changes: demo paydirect collections are matched by
--   their PRN or settlement reference against the demo students' own payment
--   references (still present at that point — payment_reference is deleted
--   later in the same transaction). Everything else is identical to V129.
-- ═══════════════════════════════════════════════════════════════════════════
