-- ═══════════════════════════════════════════════════════════════════════════
-- V182 — the merit list is UTME only for now (Direct Entry set aside)
--
--   Direct Entry is not being processed this cycle — only UTME. The merit list
--   was including the Direct-Entry stream too (its 20% quota), so "Number On
--   Merit List" came out above the UTME quota (e.g. 128 = 120 UTME + 8 DE) and
--   did not match the UTME quota distribution. Restrict the pool to UTME entry
--   mode so Direct-Entry (and Transfer) candidates do not appear on the merit
--   list, get no proposed offer, and are simply left for later. Everything else
--   is exactly as V181 (non-index computed screening, NM/SM/ELG/Locality).
--
--   Reversible: a later migration drops the entry-mode filter to bring Direct
--   Entry back into the engine.
-- ═══════════════════════════════════════════════════════════════════════════
