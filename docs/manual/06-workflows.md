...
A matriculated student asks to defer a semester or a whole session; the department recommends, the faculty recommends, the Registry approves; a daily clock brings the period into force; the return is confirmed. Every turn is on a write-once trail (`people.deferment_event`) and tells the student by email and SMS.

```text
Student: /student/deferment      Desks: /deferments · /deferments/{id} · /deferments/returns
DRAFT → SUBMITTED → DEPT_RECOMMENDED → FAC_RECOMMENDED → APPROVED → ACTIVE → COMPLETED
        side exits: CORRECTION_REQUIRED (back to the student) · REJECTED · CANCELLED
return status: UPCOMING → DUE (within reminder_days) → OVERDUE (past return_on + overdue_after_days) → RETURNED
```

| Stage | Who starts / acts | Precondition / validation | Data created / changed | Status change | Notification | Document generated | On rejection / return / lapse |
|---|---|---|---|---|---|---|---|
| Open / edit | Student (wizard step 1: type SEMESTER / SESSION, session, semester, reason, explanation) | `people.deferment_eligibility`: "A graduated student does not defer."; WITHDRAWN / EXPELLED / TRANSFERRED_OUT / DECEASED / RUSTICATED / SUSPENDED refused; no matric number ("Deferment is asked for once you are matriculated…"); "You already have a deferment request … One request is decided before another is made."; "You have deferred N session(s) in all; the University allows M." (`max_sessions` 2; a session counts 1, a semester 0.5); "a session that has passed is not deferred" | `people.deferment` (reference `DEF-YYYY-NNNNN`, `period_from`, return session/semester/date); event CREATED / UPDATED | DRAFT | none | — | Editing only in DRAFT / CORRECTION_REQUIRED |
| Documents | Student (step 2; PDF/JPEG/PNG ≤ 5 MB, at most six) | `DEF_DOC_TYPE` (magic bytes), `DEF_DOC_SIZE`, `DEF_DOC_MANY`, `DEF_DOC_CLOSED` (only while DRAFT / CORRECTION_REQUIRED / SUBMITTED) | `deferment_document` + blob | — | none | — | `verified_at/by` columns exist but nothing sets them |
| Submit | Student (step 3, declaration) | "explain the reason in a few sentences" (≥ 20 chars when the reason needs words); "a medical deferment is supported by a document"; "confirm the declaration before submitting" | `deferment_submit` | DRAFT → SUBMITTED | Student Email + SMS "Your deferment request has been received"; HOD Email "A deferment request awaits the department" | — | — |
| Department recommends | HOD of the student's department ("Recommend to the Faculty") | `DEF_NOT_YOUR_STAGE` ("This request is submitted; approve is not this desk's act at that stage."); bound → 403 "This deferment is outside your office's bound…" | `dept_at/by/note`; event | SUBMITTED → DEPT_RECOMMENDED | Student "…recommended by the department"; Dean "A deferment request awaits the faculty" | — | REJECT (note required: "a rejection carries its reason") → REJECTED, "Your deferment request has been rejected"; CORRECTION ("say what the student must correct") → CORRECTION_REQUIRED, "Your deferment request requires correction" — the student edits and resubmits |
| Faculty recommends | Dean or Faculty Officer of the faculty ("Recommend to the Registry") | as above | `fac_*` | DEPT_RECOMMENDED → FAC_RECOMMENDED | Student "…recommended by the faculty"; Academic Office "A deferment request awaits approval" | — | as above |
| Approve | Registry (`academic`, `registrar`, `dregistrar`, `super`) — may act at any stage from either recommended state | — | `decided_*`; runs `deferments_tick` at once | → APPROVED | Student "Your deferment request DEF-… has been approved" | Approval letter PDF (student `/student/deferment/letter/{id}`, desk `/deferments/{id}/letter`; 409 "The letter is issued once the deferment is approved." before) — its QR points to `/verify/deferment/{reference}`, **a route that does not exist** | Cancel (Registry only; the student's own withdrawal passes "Withdrawn by the student"): from DRAFT…APPROVED with a note; "a deferment in force is not cancelled; the student returns from it" → CANCELLED, "Your deferment request has been cancelled" |
| In force | the clock `DefermentClock` daily 06:20 Africa/Lagos (`moaum.deferments.cron`), or `POST /deferments/tick` (registrar, dregistrar, academic, ict, super) | APPROVED with `period_from` reached (or the session CURRENT and the semester OPEN) | `prior_status` saved; `people.change_status(…,'DEFERRED', reference, …)` for matriculated ACTIVE / PROBATION / ADMITTED students; event ACTIVATED | APPROVED → ACTIVE; student → DEFERRED | Student "Your deferment is now in force" | — | Registration gates: `deferment_gate_course_registration`, `deferment_gate_pg_registration`, `deferment_gate_college_semester/year` refuse "REGISTRATION UNAVAILABLE: your deferment for % is approved; you cannot register for the deferred period" unless `moaum.deferment_override = on` (SQL level only) |
| Reminder | the clock | ACTIVE with `return_on` within `reminder_days` (14) and no reminder yet | `reminder_sent_at` | — | Student "Your deferment is ending soon"; HOD "A student is due to return from deferment" | — | — |
| Confirm return | HOD, Dean, Faculty Officer or Registry on Returns `/deferments/returns` → review | on ACTIVE or APPROVED | `deferment_confirm_return`: `change_status` back to `prior_status` (ACTIVE / PROBATION / ADMITTED, else ACTIVE); event RETURNED | → COMPLETED; student restored | Student "Welcome back — your return from deferment is confirmed" | — | OVERDUE after `overdue_after_days` (30) is a filter, not an act |

Desk exports: branded Excel / PDF (serial DEF, S/N first). Settings (`people.deferment_setting`: max_sessions 2, allow_extension true, reminder_days 14, overdue_after_days 30) and reasons (`people.deferment_reason`) have no screen.
