...
| POST | `/api/v1/admissions/sessions/{session}/{year}/eligibility/recalculate-all` | evaluate up to `limit` (default 50, at most 500) submitted applications — with `onlyMissing=true` only those without a current evaluation — and return `evaluated` and `remaining`; quiet (OFFICER) unless `notify=true` (SYSTEM) | academic, dregistrar, registrar, super | `admissions/AdmissionEligibilityController.java:208` |
