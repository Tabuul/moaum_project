...
| Deferments (`/deferments/{id}`) | Department → Faculty → Registry desks by stage | Java `may()` + bound | IMPLEMENTED |
...
**3.1.5 Deferments.** Status: IMPLEMENTED (V259). Screen: Deferments — `/deferments`, review `/deferments/{id}`, returns `/deferments/returns`. The Registry (`academic, registrar, dregistrar, super`) approves at DEPT_RECOMMENDED or FAC_RECOMMENDED, may reject or ask for correction at its own stage, is the only desk that may **Cancel**, and confirms returns. Approval runs the clock at once; the daily clock (06:20 Africa/Lagos) makes an approved deferment ACTIVE on its start date, sets the student DEFERRED, refuses registration by trigger for the period, reminds 14 days before the return and flags overdue returns 30 days after. Settings (`max_sessions` 2, `allow_extension`, `reminder_days` 14, `overdue_after_days` 30) and the reason list are tables with **no screen**. The approval letter's QR points to `/verify/deferment`, which **does not exist** — NOT IMPLEMENTED. Desk exports: Excel/PDF (serial DEF, S/N first).
...
8. **Reconsiderations** (`/admissions`): non-qualified candidates with five credits and an open programme they qualify for; **Suggest** emails "Your … admission — a suggested programme".
...
**What the VC acts on.** Senate Business — `/results/approvals` (read; the minute is the Registrar's); Graduation (read); Audit Trail, Data Governance (read), Security Posture; Budget (read); revoking a digital document (with the Registrar); HR movement approvals (`vc`, `dvc`); transfers (`dvc` in the legacy Senate path only — no screen). The DVC additionally reads the Senate Schedule, Result Pipeline and Broadsheets, and Deferments are not on the DVC's list. Neither office can keep or file a return except through the snapshot readers' guard, which both hold.
...
| Deferment settings and reasons (`people.deferment_setting`, `deferment_reason`) | migration | ICT | max 2 sessions; extension on; remind 14 d; overdue after 30 d; 9 reasons | No screen |
