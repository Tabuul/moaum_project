...
**2.12.x Matriculation Management (V267).** **Status: IMPLEMENTED.** Screen: Matriculation Management — `/matriculation/manage?session=&fac=` (menu: academic, registrar, dregistrar and facultyofficer, Students group; also from Number format). Readers add dvc, vc, records, dean, ict, admin, super; preparers (generate, validate, correct, drop, mark ready) academic, registrar, dregistrar, facultyofficer — a Faculty Officer or Dean is held to their own faculty by the office scope and never sees the all-faculties view; issuers (issue, cancel) academic, registrar, dregistrar.

*The rule.* Generating a matriculation number is preparation; issuing it is the official act. Nothing reaches a student record until an authorised officer confirms the issue.

*Faculty view.* Choose the session and the faculty (Programme, Status and Search narrow it on the server; the search reads name, student ID, admission, application and matriculation numbers, programme and series). KPI tiles — Eligible, Prepared, Pending, Already matriculated, Numbers generated, Numbers issued, Conflicts, Faculty list — each a door to its list. Students are grouped by programme (collapsible), each group a table S/N · Student ID · Student Name · Programme · Year · Series · Proposed Matric No. · Status · Validation · Action, names A–Z. Eligible means admitted, no number, an approved course registration and settled fees for the session, not under query on the faculty list, and a buildable number under the configured rule; anyone else is *Pending* with the reason.

*The batch.* **Generate Matriculation Numbers** opens (or reuses) the faculty's one open batch — reference `MAT/YYYY/NNN` from the run series — and proposes a number for every eligible student (or one programme) from the V263 rule, reserving each sequence in its series so no other batch or the straggler act can take it; the series' `last_issued` does not move. Every row is validated (student exists, admitted, no number, in the faculty and programme, eligible; shape, the student's own segments, positive sequence, unique against the register, the history and other batches' reservations, no duplicate within the batch, series active) and re-validated at every act. **Fix / Edit** corrects a proposed number with a required reason — validated the same way, the previous value, who and when kept on the batch's edit history — and **Drop** takes a student off with a reason, releasing the number. **Mark ready for issuance** needs zero conflicts. States: DRAFT → GENERATED (under review) → READY FOR ISSUANCE → ISSUED, or CANCELLED (reason required; reserved numbers released; an issued batch is never cancelled).

*All faculties.* The Registry's tab: Final matriculation review tiles (faculties, programmes, students prepared, valid, conflicts, already matriculated, ready, issued) and a table per faculty with Prepared · Valid · Conflicts · Issued and the batch state; **Issue** per ready batch or **Confirm & Issue every READY batch**.

*Issuing.* From the final review (session, faculty, students, valid, conflicts, ready) → the confirmation dialog listing what happens → **Confirm & Issue** (`POST …/issue {confirm:true}`; refused without the confirmation, on any batch not READY, or while a conflict stands). One transaction over the batch: each student verified again, the number written to the history and the register, the series moved forward, status ADMITTED → ACTIVE with its status change, the sign-in identity moved from the admission number to the matriculation number on the same account with the same password (`people.student_username_change`), the reservation released, the notice sent ("you are officially matriculated … your matriculation number is now your portal sign-in username"). A single failure issues nothing. The completion dialog shows Successfully matriculated / Failed / Username updates and the post-issuance verification (number on the record, status active, history, username history, uniqueness). With **Separation of duties** on (Number format), the officer who generated or marked the batch ready cannot issue it.

*After.* The student signs in with the matriculation number; the admission number no longer opens the portal. Student 360 shows a Matriculation panel — the number, the date, the batch, and Before / After (username, status, number) with the username history. Tabs *Batches* (every batch with its officers and counts), *Issued* (S/N · Matric No. · Student Name · Faculty · Department · Programme · Session · Username · Issued · Batch; filters faculty, programme, batch, search) and *Pending* (S/N · Student · Faculty · Programme · Reason).

*Reports* (Excel and PDF, S/N first, names A–Z): Faculty Matriculation Report, Programme Matriculation Report (per group), Issued Matriculation Numbers, All-Faculty Matriculation Report, Pending Matriculation Report.

> **Screenshot Required:** Matriculation Management — `/matriculation/manage?fac=MS` — the eight tiles, the batch panel with Mark ready / Final review & issue, and a programme group with proposed numbers and their validation.
...
**2.13.7 Online screening and the admission lifecycle (V269).** **Status: IMPLEMENTED.** Screen: Screening Review — `/admissions/screening-review?session=` (menu: academic and registrar, Admissions group; from the pipeline tiles on Admissions). Readers: academic, registrar, dregistrar, records, dean, facultyofficer (bound to their faculty), hod, dvc, vc, ict, admin, super; deciding: academic, registrar, dregistrar, super.

*The policy.* Per session: on or off, from when (an applicant who accepted before the switch is not held), the documents and the fields the form requires, instructions. Every session with admission settings screens from 26 September 2026. Edited under **Screening policy** on the desk.

*The form* is the Registry's paper screening on the application: answers on the student biodata catalogue (the forms' fields the catalogue lacked were added — maiden name, number of children, hobbies, postal address, parent's profession and income, primary and secondary school with fees per term, graduation years, A-Level institution, working experience, memberships), the institutions attended, the O'Level results as declared beside JAMB's, the documents on the existing application document (kinds widened), the declaration, a write-once trail. Opened on acceptance (SCR/YYYY/NNNNNN), saved as a draft, submitted only when every required field and document and at least one O'Level row stand; read-only after; the applicant and the Academic Office are told.

*The desk.* Tiles — Total, Pending (not submitted), Pending review, In review, Successful, Unsuccessful, Returned, Change of programme requested, Completed, Overdue (undecided seven days after submission) — each a door to its list; filters status, faculty, department, programme, submitted between, search over name, JAMB, application and screening numbers and programme, on the server. **Review** opens the form in full: the tracker, what is on record (sex, date of birth, state, UTME score and subjects), every section of answers, the institutions, the O'Level declared beside JAMB's, the documents (opened through the office's authorised door), the change requests, the trail. Decisions: **Approve screening** (the application is cleared — the journey's stage 7 —, the answers go onto the student record, the applicant is told they may pay school fees and register), **Unsuccessful** (reason required; the eligibility engine reads the record and lists the programmes the candidate qualifies for; the applicant is told the reason and the count), **Request correction** (note required; the form returns to the applicant). Excel and PDF of the queue as filtered.

*The gates.* Where the policy requires it: no school-fee reference (`finance.new_reference`) and no course registration for a student whose screening is not successful and whose change of programme is not approved ("SCHOOL FEES UNAVAILABLE" / "COURSE REGISTRATION UNAVAILABLE"); Matriculation Management names the screening as the pending reason. Students who came onto the register without an application of this portal are not held.

*Change of programme after an unsuccessful screening.* The existing change-of-programme queue (Programme Eligibility, §2.13.6) takes the applicant's request after the Board's decision only in that case, once; the Office's approval moves the candidate and the student on the register (not yet matriculated) to the new programme, clears the application and tells the applicant that the acceptance fee — paid once, for the admission — is not charged again (`admissions.acceptance_entitlement`).

*The lifecycle.* `admissions.admission_status(application)` gives the status (ADMITTED, ACCEPTANCE_PENDING, SCREENING_PENDING / IN_PROGRESS / RETURNED / SUBMITTED, CHANGE_OF_PROGRAMME_REQUIRED / PENDING, REGISTER_PENDING, SCHOOL_FEES_PENDING, COURSE_REGISTRATION_PENDING, MATRICULATION_PENDING, MATRICULATED; NOT_ADMITTED, DECLINED, PENDING before), the label, the next action and its door; `admission_tracker` the steps. The Admissions page carries the pipeline tiles (JAMB admissions uploaded and matched, admitted, acceptance pending and paid, screening pending / to review / successful / unsuccessful, change of programme requested and approved, school fees paid, registered, ready for matriculation, matriculated), each a door.

*Matriculation broadcast.* On Matriculation Management → Batches, **Broadcast** on an issued batch sends every student the matriculation message (name, number, faculty, department, programme, session, the sign-in instruction; no password) by email and SMS and records the send (`people.matric_broadcast`); **Retry failed** sends again only where the notice failed.

> **Screenshot Required:** Screening Review — `/admissions/screening-review` — the ten tiles, the queue, and the review dialog with the O'Level declared beside JAMB's and the three decision buttons.
