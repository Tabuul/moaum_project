...
| `screening_answer` | an answer on the online screening form, keyed on `ref.biodata_field` (V269) | application_id, field | application, ref.biodata_field | 3 |  | 0 | exempt | V269 |
| `screening_event` | the write-once trail of the online screening: opened, saved, submitted, review started, returned, decided, programme changed | id | application | 7 |  | 0 | exempt (write-once) | V269 |
| `screening_form` | ── the online screening of a fresh student (V269) ─────────────────────── DRAFT → SUBMITTED → UNDER_REVIEW → SUCCESSFUL / UNSUCCESSFUL / RETURNED; SCR/YYYY/NNNNNN; the declaration, the decision and its reason | application_id | application | 19 |  | 1 | attached | V269 |
| `screening_institution` | an institution attended with dates and the certificate (Section B); deactivated, never deleted | id | application | 9 | (application, ord) | 0 | exempt | V269 |
| `screening_olevel` | an O'Level result as the candidate declares it (Section C), beside JAMB's | id | application | 10 | (application, ord) | 0 | exempt | V269 |
| `screening_policy` | per session: online screening on/off, from when, the documents and fields required | session | policy.academic_session | 7 |  | 1 | attached | V269 |
...
| `matric_batch` | ── one matriculation exercise for a faculty and session (V267) ───────── DRAFT → GENERATED → READY_FOR_ISSUANCE → ISSUED / CANCELLED; ref from the run series; preparer, reviewer, issuer; one open per faculty | id | policy.academic_session, ref.faculty, matriculation_run | 25 | (session, faculty open) | 1 | attached | V267 |
| `matric_broadcast` | a matriculation broadcast to an issued batch: who sent it, when, to how many, whether only the failed (V269) | id | matric_batch | 8 |  | 0 | attached | V269 |
| `matric_batch_edit` | every correction of a proposed number: previous, new, reason, who, when; written once | id | matric_batch, matric_batch_row, student | 10 |  | 0 | attached | V267 |
| `matric_batch_row` | a student on a batch: the proposed number, the generated one, the series and sequence, edited/previous/reason, the validation problems, PROPOSED / ISSUED / DROPPED | id | matric_batch, student, matric_series | 22 | (batch, student) | 1 | attached | V267 |
| `matric_reservation` | the sequence a batch row holds while the batch is prepared; released (never deleted) on issue, drop or cancel; unique per series+sequence and per number while unreleased | id | matric_series, matric_batch, matric_batch_row | 10 | (series, sequence) where open | 0 | exempt | V267 |
...
| `student_username_change` | the student's sign-in identity each time it changes — the admission number before, the matriculation number after — with the reason, the officer and the batch; written once (V267) | id | student, matric_batch | 9 |  | 0 | attached | V267 |
