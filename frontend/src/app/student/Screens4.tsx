        ...
        <>
          ...
          {latest ? (
            <Panel title={`${latest.session} · ${semesterName(latest.semester)} semester`} right={live.length ? `Published${live[0]?.senate_minute ? ` under ${live[0].senate_minute}` : ""}` : "In the approval chain"}>
              <DTable cols={["Course", "Units|mid", "Score|mid", "Grade|mid", "Points|mid", "Where it is", "Taught by"]} rows={latest.rows.map((c) => {
                const st = STAGE_LABEL[c.stage] ?? [c.stage, ""];
                const taught = <span className="sub2" key="h">{c.lecturer || "—"}</span>;
                if (!c.published) return [<Two key="c" a={<span className="tnum">{c.course_code}</span>} b={c.title} />, <span className="tnum" key="u">{c.units}</span>, <span className="sub2" key="s">—</span>, <span className="sub2" key="g">—</span>, <span className="sub2" key="p">—</span>, <Pil kind="info" key="w">{st[0]}</Pil>, taught];
                if (c.outcome !== "GRADED") return [<Two key="c" a={<span className="tnum">{c.course_code}</span>} b={c.title} />, <span className="tnum" key="u">{c.units}</span>, <Pil kind="bad" key="s">{c.outcome === "ABSENT" ? "Absent" : c.outcome}</Pil>, <span className="sub2" key="g">—</span>, <span className="sub2" key="p">—</span>, <Pil kind="ok" key="w">Published</Pil>, taught];
