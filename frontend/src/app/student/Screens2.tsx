              ...
              (() => {
                ...
                return (
                  <>
                    ...
                    <div className="row">
                      <Btn kind="primary" disabled={busy !== null} onClick={() => void act("ref", "POST", "/me/fees/references", { session: fees.session, amount: chosen.amount }, `Fee reference generated by the student for ${fees.session}`)}>{busy === "ref" ? "Generating…" : `Generate a reference for ${naira(chosen.amount)}`}</Btn>
                    </div>
                  </>
                );
              })()
      ...
      <div className="doc" style={{ maxWidth: 660 }}>
        ...
        <div className="tablewrap"><table style={{ minWidth: 400 }}>
          ...
          <tbody>
            ...
            <tr style={{ background: "var(--bg)" }}><td className="b700">TOTAL RECEIVED</td><td className="num tnum" style={{ fontWeight: 700, fontSize: 16 }}>{naira(r.amount)}</td></tr>
          </tbody>
        </table></div>
