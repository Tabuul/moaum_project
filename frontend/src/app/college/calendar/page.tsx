...
export default async function CollegeCalendarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const calendar = session ? await api<{ session: string; rows: CalendarRow[] }>(`/api/v1/college/calendar?session=${encodeURIComponent(session)}`) : null;
  ...
  return (
    <Shell route="t/collegecalendar" me={me.ok ? me.data : null}>
      {calendar && !calendar.ok ? <ProblemNotice problem={calendar.problem} /> : (
        <Calendar sessions={sessionList.map((s) => s.name)} session={session} rows={calendar && calendar.ok ? calendar.data.rows : []} mayEdit={mayEdit} problem={null} />
      )}
    </Shell>
  );
}
