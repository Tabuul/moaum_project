...
import { ScoreSheets, type Upload } from "./ScoreSheets";
import type { Candidates, ExamCatalogue, ExamSummary } from "../examinations/Examinations";
...
export default async function ScoreSheetsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  // the desk opens on the level that has a cohort in the session; the coordinator on their own
  const summary = session ? await api<ExamSummary[]>(`/api/v1/college/exams/summary?session=${encodeURIComponent(session)}`) : null;
  const firstWithCohort = summary && summary.ok ? summary.data.find((x) => Number(x.cohort) > 0)?.level : undefined;
  const level = mine && mine.ok ? mine.data.level : typeof p.level === "string" && p.level ? Number(p.level) : (firstWithCohort ?? 200);
  const exam = catalogue.ok ? catalogue.data.exams.find((e) => e.level === level) : null;
  const [candidates, uploads] = await Promise.all([
    session && exam ? api<Candidates>(`/api/v1/college/exams/${encodeURIComponent(exam.code)}/candidates?session=${encodeURIComponent(session)}`) : null,
    session && exam ? api<Upload[]>(`/api/v1/college/exams/${encodeURIComponent(exam.code)}/uploads?session=${encodeURIComponent(session)}`) : null,
  ]);
  return (
    <Shell route="t/collegesheets" me={me.ok ? me.data : null}>
      {!catalogue.ok ? <ProblemNotice problem={catalogue.problem} /> : mine && !mine.ok ? <ProblemNotice problem={mine.problem} /> : (
        <ScoreSheets sessions={sessionList.map((s) => s.name)} session={session} level={level} exams={catalogue.data.exams} coordinator={coordinator} summary={summary && summary.ok ? summary.data : []}
          data={candidates && candidates.ok ? candidates.data : null} problem={candidates && !candidates.ok ? candidates.problem : null}
          uploads={uploads && uploads.ok ? uploads.data : []} />
      )}
    </Shell>
  );
}
